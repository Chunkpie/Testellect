...
from app.services.pdf_paper_service import ensure_fonts, FONTS_DIR
...
class PDFReportService:
    @staticmethod
    async def generate_school_summary(
        db: AsyncSession, school_id: int, report_id: int
    ) -> str:
        ...
        # Title
        elements.append(
            Paragraph(f"School Performance Report: {school.name}", styles["Title"])
        )
        ...
        for a in assessments:
            ...
            avg = (
                round(
                    sum(r.percentage for r in s_results if r.percentage)
                    / len(s_results),
                    2,
                )
                if s_results
    ...
    @staticmethod
    async def generate_omr_results_report(
        db: AsyncSession,
        student_id: int,
        batch_id: str,
        ...
        elements.append(
            Paragraph(
                f"OMR Results - {student_name} (ID: {student_id})", styles["Title"]
            )
        )
        ...
        elements.append(
            Paragraph(f"Score: {score}/{max_score} ({percentage}%)", styles["Heading2"])
        )
        elements.append(Spacer(1, 20))
        ...
        for ans in evaluated_answers:
            ...
            if is_correct == "Right":
                result_cell = Paragraph(
                    f"<font color='green'>{is_correct}</font>", styles["Normal"]
                )
            else:
                result_cell = Paragraph(
                    f"<font color='red'>{is_correct}</font>", styles["Normal"]
                )
    ...
    async def generate_student_report(
        ...
        if language.lower() == "hindi" and "HindiFont" in pdfmetrics.getRegisteredFontNames():
            font_name = "HindiFont"
            font_name_bold = "HindiFont"
        elif language.lower() == "gujarati" and "GujaratiFont" in pdfmetrics.getRegisteredFontNames():
            font_name = "GujaratiFont"
            font_name_bold = "GujaratiFont"
        ...
        details_data = [
            ...
            [
                ...
                Paragraph(
                    (
                        f"Grade {classroom.grade} - {classroom.section}".upper()
                        if classroom
                        else "N/A"
                    ),
                    ParagraphStyle(
            ...
            [
                [
                    [
                        ...
                        Paragraph(
                            f"Assessment: {assessment.name if assessment else 'General performance evaluation'}",
                            ParagraphStyle(
                                "AS",
                                fontName="Helvetica",
                                fontSize=9,
        ...
        if not insights:
            elements.append(
                Paragraph(
                    "No detailed learning analytics available yet for this student.",
                    body_style,
                )
            )
            doc.build(elements)
        ...
        if not narrative_paras:
            narrative_paras = [
                Paragraph("No narrative evaluation available.", body_style)
            ]
        else:
        ...
        strengths_paras = [
            Paragraph("Strengths & Mastered Concepts", strength_title_style)
        ]
        for s in insights.get("strengths", []):
            strengths_paras.append(Paragraph(f"• {s}", bullet_style))
        if len(insights.get("strengths", [])) == 0:
            strengths_paras.append(
                Paragraph("• No specific strengths identified yet.", bullet_style)
            )

        improvement_paras = [
            Paragraph("Areas for Improvement", improvement_title_style)
        ]
        ...
        if len(insights.get("areas_for_improvement", [])) == 0:
            improvement_paras.append(
                Paragraph("• No immediate improvement areas identified.", bullet_style)
            )

        side_outer_table = Table(
            [[strengths_paras, "", improvement_paras]], colWidths=[250, 15, 250]
        )
        side_outer_table.setStyle(
            TableStyle(
                [
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
                    ("LEFTPADDING", (0, 0), (-1, -1), 0),
        ...
        if len(insights.get("recommendations", [])) == 0:
            rec_paras.append(
                Paragraph(
                    "• Continue regular practice and review correct concepts.",
                    bullet_style,
                )
            )
