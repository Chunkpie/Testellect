...
from app.db.models.mcq_engine import (
    MCQEQuestionPaper, MCQEQuestionPaperItem, MCQEStudentResult, 
    MCQEStudentAnswerDetail, MCQEStudentConceptAnalysis
)
...
async def evaluate_omr_sheet(db: AsyncSession, image_path: str) -> int:
    ...
    res_p = await db.execute(select(MCQEQuestionPaper).where(MCQEQuestionPaper.paper_code == paper_code))
