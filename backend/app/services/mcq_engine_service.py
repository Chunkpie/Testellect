...
async def run_generation_job(job_id: int):
    async with async_session_factory() as db:
        ...
        try:
            ...
            # 1. Parse Textbook
            res = await db.execute(
                select(MCQETextbook).where(MCQETextbook.id == job.textbook_id)
            )
            ...
            if not os.path.exists(textbook.file_path):
                ...
                chunks = await asyncio.to_thread(
                    extract_text_from_pdf, textbook.file_path
                )
            ...
            for chunk in limit_chunks:
                try:
                    ...
                    res_ollama = await ollama.generate_structured(
                        ...
                        temperature=0.1,
                        num_ctx=6000,
                        model=AGENT_MODEL_MAP["mcq_engine"],
                    )

                    embed = [random.random() for _ in range(10)]

                    res_ec = await db.execute(
                        select(MCQEConcept).where(
                            MCQEConcept.textbook_id == textbook.id
                        )
                    )
                    ...
                    if not is_duplicate:
                        concept = MCQEConcept(
                            ...
                            concept_summary_text=res_ollama.get(
            ...
            if not unique_concept_ids:
                ...
                concept = MCQEConcept(
                    ...
                    chapter_name="Test",
                    concept_title="Test",
                    concept_summary_text="Test Summary",
                )
                db.add(concept)
                await db.commit()
                await db.refresh(concept)
            ...
            while generated_count < target_count:
                for cid in unique_concept_ids:
                    ...
                    res_c = await db.execute(
                        select(MCQEConcept).where(MCQEConcept.id == cid)
                    )
                    ...
                    try:
                        target_langs = [l.strip() for l in job.language.split(",")] if "," in job.language else [job.language]
                        if job.language.lower() == "all":
                            target_langs = ["English", "Hindi", "Gujarati"]
            ...
            res_m = await db.execute(
                select(MCQEMcq).where(MCQEMcq.generation_job_id == job.id)
            )
