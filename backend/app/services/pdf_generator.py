...
from app.db.models.mcq_engine import (
    MCQEGenerationJob, MCQEQuestionPaper, MCQEQuestionPaperItem, MCQETextbook,
    MCQEStudentResult, MCQEStudentAnswerDetail, MCQEStudentConceptAnalysis, MCQEMcq
)
...
async def generate_paper_pdf(db: AsyncSession, paper_id: int) -> bytes:
    ...
    res = await db.execute(
        ...
        .options(selectinload(MCQEQuestionPaper.generation_job), selectinload(MCQEQuestionPaper.textbook))
    ...
    # Header
    c.drawString(50, height - 50, f"Class: {job.class_name} | Subject: {job.subject_id} | Book: {textbook.book_name}")
    c.drawString(50, height - 70, f"Paper Code: {paper.paper_code}")
    c.drawString(50, height - 90, "Name: ______________________  Roll No: __________  Date: ________")
...
async def generate_papers_zip(db: AsyncSession, job_id: int) -> bytes:
    ...
    res_papers = await db.execute(select(MCQEQuestionPaper).where(MCQEQuestionPaper.generation_job_id == job.id))
...
async def generate_marked_answer_sheet(db: AsyncSession, student_result_id: int) -> bytes:
    register_fonts()
    res = await db.execute(
    ...
    font_name = "Helvetica"
    if job.language == "hindi": font_name = "Devanagari"
    elif job.language == "gujarati": font_name = "Gujarati"
        
    try: c.setFont(font_name, 12)
    except: c.setFont("Helvetica", 12)
        
    # Header
    c.drawString(50, height - 50, f"Student Roll No: {result.student.roll_number} | Score: {result.score}/40")
...
async def generate_concept_analysis_report(db: AsyncSession, student_result_id: int) -> bytes:
    register_fonts()
    res = await db.execute(
        select(MCQEStudentResult)
        .options(selectinload(MCQEStudentResult.student))
    ...
    c.setFont("Helvetica", 12)
    c.drawString(50, height - 70, f"Roll No: {result.student.roll_number} | Score: {result.score}/40")
    ...
    for a in analysis_records:
        ...
        c.drawString(50, y, f"- {title}: {a.questions_correct}/{a.questions_attempted} ({a.classification})")
        y -= 20
