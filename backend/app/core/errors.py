...
class NotFoundError(AppError):
    def __init__(
        ...
        super().__init__(
            code="NOT_FOUND", message=message, details=details, status_code=404
        )
...
class ValidationError(AppError):
    def __init__(self, message: str = "Validation failed", details: dict | None = None):
        super().__init__(
            code="VALIDATION_ERROR", message=message, details=details, status_code=422
        )
...
class AIProcessingError(AppError):
    def __init__(
        self, message: str = "AI processing failed", details: dict | None = None
    ):
        super().__init__(
