...
async def export_students(
    ...
    stmt = select(Student).where(Student.school_id == target_school, Student.is_deleted == False)
...
async def export_assessment_results(
    ...
    stmt = select(StudentResult, Student).join(Student).where(StudentResult.assessment_id == assessment_id)
    result = await db.execute(stmt)
    rows = result.all()
    ...
    for sr, student in rows:
        data.append({
            ...
            "Percentage": f"{sr.percentage}%" if sr.percentage is not None else "N/A"
        })
