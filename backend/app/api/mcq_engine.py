...
async def upload_curriculum(
    ...
    curriculum = MCQEngineCurriculum(
        class_name=curr_in.class_name, subject_id=curr_in.subject_id
    )
    db.add(curriculum)
    await db.commit()
...
async def download_papers(
    ...
    return Response(content=zip_bytes, media_type="application/x-custom-zip")
...
async def download_marked_sheet(
...
async def download_concept_analysis(
    student_result_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
