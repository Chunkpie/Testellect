...
def _validate_distributions(data: BlueprintCreate):
    ...
    if data.difficulty_distribution:
        try:
            ...
            if abs(total_pct - 100) > 1:
                errors.append(f"Difficulty distribution sums to {total_pct}%, expected 100%")
    ...
    if data.competency_distribution:
        try:
            ...
            total_pct = sum(dist.values())
            if abs(total_pct - 100) > 1:
                errors.append(f"Competency distribution sums to {total_pct}%, expected 100%")
        except (json.JSONDecodeError, TypeError):
...
async def create_blueprint(
    ...
    if current_user.role not in ("teacher", "administrator"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only teachers and administrators can create blueprints")
    ...
    await log_audit_entry(
        db=db, user_id=current_user.id, school_id=bp.school_id,
        action="create", resource_type="blueprint", resource_id=bp.id,
    )
...
async def update_blueprint(
    ...
    if current_user.role not in ("teacher", "administrator"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only teachers and administrators can update blueprints")
    ...
    paper_result = await db.execute(select(Paper).where(Paper.blueprint_id == blueprint_id).limit(1))
    ...
    await log_audit_entry(
        db=db, user_id=current_user.id, school_id=bp.school_id,
        action="update", resource_type="blueprint", resource_id=bp.id,
    )
...
async def check_coverage(
    ...
    if current_user.role not in ("teacher", "administrator"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only teachers and administrators can check coverage")
...
async def generate_paper_from_blueprint(
    ...
    if user.role not in ("teacher", "administrator"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only teachers and administrators can generate papers")
    ...
    try:
        bloom_dist = json.loads(blueprint.bloom_distribution) if blueprint.bloom_distribution else {}
        difficulty_dist = json.loads(blueprint.difficulty_distribution) if blueprint.difficulty_distribution else {}
    except (json.JSONDecodeError, TypeError):
        bloom_dist = {}
        difficulty_dist = {}

    bloom_pool = list(bloom_dist.keys()) if bloom_dist else ["remember", "understand", "apply", "analyze", "evaluate"]
    difficulty_pool = list(difficulty_dist.keys()) if difficulty_dist else ["easy", "medium", "hard"]
    ...
    for bloom_level in bloom_pool:
        if remaining <= 0:
            break
        bloom_weight = bloom_dist.get(bloom_level, 0) / 100.0 if bloom_dist else 1.0 / len(bloom_pool)
        target = max(1, int(blueprint.total_questions * bloom_weight))
        pool = [q for q in all_questions if q.bloom_level == bloom_level and q not in selected]
        random.shuffle(pool)
    ...
    for diff in difficulty_pool:
        if remaining <= 0:
            break
        diff_weight = difficulty_dist.get(diff, 0) / 100.0 if difficulty_dist else 1.0 / len(difficulty_pool)
    ...
    await log_audit_entry(
        db=db, user_id=user.id, school_id=blueprint.school_id,
        action="generate", resource_type="paper", resource_id=paper.id,
    )
...
async def delete_blueprint(
    ...
    await log_audit_entry(
        db=db, user_id=current_user.id, school_id=bp.school_id,
        action="delete", resource_type="blueprint", resource_id=blueprint_id,
    )
