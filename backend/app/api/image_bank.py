...
async def upload_image(
    ...
    if current_user.role not in ("teacher", "administrator"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only teachers can upload images")
...
async def list_images(
    ...
    if grade:
        stmt = stmt.where(ImageAsset.grade_range_min <= grade, ImageAsset.grade_range_max >= grade)
...
async def delete_image(
    ...
    if current_user.role not in ("administrator", "deo") and asset.school_id != current_user.school_id:
