...
async def district_analytics(
    ...
    if school_ids:
        result = await db.execute(
            select(Assessment).where(Assessment.school_id.in_(school_ids))
        )
    ...
    if assessment_ids:
        result = await db.execute(
            select(StudentResult).where(StudentResult.assessment_id.in_(assessment_ids))
        )
...
async def school_analytics(
    ...
    result = await db.execute(
        select(Assessment).where(Assessment.school_id == school_id)
    )
    assessments = result.scalars().all()
    assessment_ids = [a.id for a in assessments]
...
async def assessment_analytics(
    ...
    result = await db.execute(
        select(StudentResult).where(StudentResult.assessment_id == assessment_id)
    )
    results = result.scalars().all()
    ...
    if results:
        ...
        comp_res = await db.execute(
            select(CompetencyResult).where(
                CompetencyResult.student_result_id.in_(res_ids)
            )
        )
        ...
        for cr in c_results:
            ...
            competency_aggregate[cr.competency_id]["attempted"] += (
                cr.questions_attempted or 0
            )
            competency_aggregate[cr.competency_id]["correct"] += (
...
async def get_competency_radar(
    ...
    if user.school_id:
        stmt = stmt.join(Assessment, OMRSheet.assessment_id == Assessment.id).where(
    ...
    q_stmt = select(QuestionBank.id, QuestionBank.bloom_level).where(
        QuestionBank.id.in_(list(question_stats.keys()))
    )
    ...
    for bloom, stats in bloom_agg.items():
        ...
        radar_data.append({"subject": bloom, "A": perc, "fullMark": 100})
